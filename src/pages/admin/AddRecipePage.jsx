...
export default function AddRecipe() {
  ...
  { name: "", quantity: "", unit: "", averagePrice: "", image: null, imagePreview: null }]);
  ...
  if (error) {
    alert("Failed to upload ingredient image!");
    setLoading(false);
    return;
  }
  ...
  setIngredients(prev =>
    prev.map((ing, i) =>
      i === index
        ? { ...ing, image: data.publicUrl, imagePreview: URL.createObjectURL(file) }
        : ing
    )
  );
    ...
    prev.map((i, idx) =>
      idx === index
        ? {
            ...i,
            name: value,
            unit: matched ? matched.unit : i.unit,
            averagePrice: matched ? matched.averageprice : i.averagePrice,
          }
        : i
    )
  );
...
const ingredientExists = (name) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Insert recipe
    const { data: recipeData, error: recipeError } = await supabase.from("recipe").insert([{
      userid: userId,
      title: title.trim(),
    ...
    if (recipeError) { alert("Failed to add recipe!"); console.error(recipeError); setLoading(false); return; }
    ...
    // Reset form
    setTitle(""); setDescription(""); setRecipeImage(null); setRecipeImagePreview(null);
    setVideoUrl(""); setEstimatedTime(""); setEstimatedCost("");
    setInstructions([{ text: "", image: null, imagePreview: null }]);
    setIngredients([{ name: "", quantity: "", unit: "", averagePrice: "" }]);
    setDietaryCategories([]); setCuisineTypes([]); setAllergies([]); setGoals([]);
  };
      ...
      <div className="max-w-5xl mx-auto py-10 px-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 bg-white p-6 rounded-xl shadow-md">
          ...
          <div>
            ...
            <textarea
              rows="3"
              placeholder="Add a description for the tutorial video"
            ...
            {ingredients.map((ing, idx)=>(
              <div key={idx} className="mb-3">
               ...
               <input
                  ...
                  value={ing.name}
                  onChange={(e) => handleIngredientNameChange(e.target.value, idx)}
                ...
                <input type="number" placeholder="Avg Price" value={ing.averagePrice} onChange={e=>setIngredients(prev=>prev.map((i,index)=>(index===idx?{...i,averagePrice:e.target.value}:i)))}
                  ...
                  <label className="flex items-center justify-center px-3 py-2 bg-gray-100 text-gray-700 rounded-md cursor-pointer text-sm hover:bg-gray-200">
                      ...
                      <input
                        ...
                        accept="image/*"
                        className="hidden"
                        onChange={(e) => handleIngredientImageUpload(e.target.files[0], idx)}
                      />
                    </label>

                  {ing.imagePreview && (
                    <img
                      src={ing.imagePreview}
                      alt="Ingredient"
          ...
          <button type="submit" disabled={loading} className="w-full bg-orange-600 text-white font-semibold px-6 py-3 rounded-md hover:bg-orange-700 transition">{loading ? "Submitting..." : "Submit Recipe"}</button>
        </form>
      </div>
    </div>
  );
