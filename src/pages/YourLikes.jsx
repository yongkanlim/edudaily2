...
export default function YourLikes() {
  ...
  useEffect(() => {
    const fetchLikedPosts = async () => {
      ...
      // Fetch all liked posts with full community post info
      ...
      // Map posts and fetch additional info (user + profile + counts)
      const posts = likesData.map(like => like.communitypost);
      ...
      // Fetch counts and user interactions
      const likeMap = {};
      const favMap = {};
      const commentMap = {};
      const userLikeMap = {};
  ...
  >
    ⭐ All Posts
            ...
            <ul className="text-sm text-gray-700 space-y-3">
                ...
                <li onClick={() => navigate("/your-likes")} className="hover:text-orange-600 cursor-pointer font-semibold text-orange-600">
                  ❤️ Your likes
                </li>
        ...
        <main className="lg:col-span-7 space-y-6">
          <h2 className="text-xl font-semibold text-gray-800 mb-4">❤️ Your Likes</h2>
          ...
          {likedPosts.length > 0 ? (
            likedPosts.map(post => {
              ...
              try {
                if (post.imageurl) {
                  ...
                  } else if (Array.isArray(post.imageurl)) images = post.imageurl;
                }
              } catch {
              ...
              return (
                ...
                >
                  ...
                  {images.length > 0 && (
                    <div className={`grid gap-3 mb-3 ${
                      ...
                      {images.map((img, index) => (
                        <div key={index} className="w-full h-48 rounded-md overflow-hidden border bg-gray-100 flex items-center justify-center">
                          <img src={img} alt={`Post Image ${index + 1}`} className="object-cover w-full h-full" />
                        </div>
                      ))}
                    </div>
                  )}
                  ...
                  <div className="flex gap-6 text-gray-700 text-sm mt-3">
                    <button
                      onClick={e => { e.stopPropagation(); toggleLike(post.postid); }}
                    ...
                    <button
                      onClick={e => { e.stopPropagation(); navigate(`/community/${post.postid}#comments`); }}
                    ...
                    <button
                      onClick={e => { e.stopPropagation(); toggleFavorite(post.postid); }}
            ...
            <p className="text-gray-500">You have not liked any posts yet.</p>
          )}
        </main>
