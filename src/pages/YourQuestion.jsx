...
export default function YourQuestions() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // fetch users & profiles for each post
      ...
      const combined = postsData.map((post) => {
        const u = usersData.find((x) => x.userid === post.userid);
        const p = profilesData.find((x) => x.userid === post.userid);
        return {
          ...post,
      ...
      // fetch counts for likes, comments, favorites & user interactions
      const likeMap = {};
      const favMap = {};
      const commentMap = {};
      const userLikeMap = {};
  ...
  const toggleFavorite = async (postid) => {
    if (userFavorites[postid]) {
      ...
      await supabase
        .from("post_favorites")
        .insert([{ postid, userid: userIdInt }]);
  ...
  >
    ⭐ All Posts
            ...
            <ul className="text-sm text-gray-700 space-y-3">
                <li onClick={() => navigate("/your-questions")} className="hover:text-orange-600 cursor-pointer font-semibold text-orange-600">
                  📝 Your questions
                </li>
        ...
        <main className="lg:col-span-7 space-y-6">
            <h2 className="text-xl font-semibold text-gray-800 mb-4">📝 Your Questions</h2>
          ...
          {posts.length > 0 ? (
            ...
            <p className="text-gray-500 text-sm">You have not posted any questions yet.</p>
          )}
        </main>
